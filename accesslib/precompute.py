...
def patches_on_disk(path_values: list, path_patches_values: list, patch_size):
    patch_shapes = []
    new_shapes = []
    patches_path = []
    ...
    for pos in tqdm(range(len(path_values))):
        ...
        for img_pos in range(len(img_patch)):
            ...
            save_image_on_disk(img_patch[img_pos], path)
            i += 1

        patches_path.append(patches_path_list)

    return patches_path, new_shapes, patch_shapes
...
if __name__ == "__main__":
    ...
    patches_path_out, new_shapes_out, patch_shapes_out = patches_on_disk(train["img_path"].values,
    ...
    pmask_path_out, _, _ = patches_on_disk(train["mask_path"].values,
    ...
    patches_path_out, new_shapes_out, patch_shapes_out = patches_on_disk(test["img_path"].values,
